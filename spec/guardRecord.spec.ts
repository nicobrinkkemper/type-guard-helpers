...
test('Should return true for objects with translations', (t) => {
  ...
  t.is(isTranslation(strObj as never), true);
});
...
test('Should return true for translations', (t) => {
  ...
  t.is(isTranslation(false as never), false);
  t.is(isTranslation(null as never), false);
  t.is(isTranslation(fnObj as never), false);
});
